...
const HotItem = (props) => {
  return (
    <TouchableOpacity
      onPress={() =>
        props.navigation.navigate('HeritageDetails', { id: props.item.item.id })
      }
      style={{
        flexDirection: 'row',
        ...
        {!isError && !isRefreshing && hotitem.length > 0 && (
          <View style={{ marginTop: 30, width: '90%', alignSelf: 'center' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              />
              <Text style={{ color: MD3Colors.error60, fontSize: 20 }}>
                热门榜
              </Text>
            </View>
            <FlatList
              data={hotitem}
              renderItem={(item) => (
                <HotItem navigation={props.navigation} item={item} />
              )}
